...
| Breakpoint Scale | 반응형 화면 크기 기준값 | 토큰 | 없음 | 4 | 데스크탑 중심 사내 시스템 특성상 우선순위 낮음 |
...
| Multi-select / Tag Input | 다중 선택·태그 입력 | 몰큘 | 없음 | 3 | 필터 등에 활용 |
...
| Pass/Fail Toggle | 합격·불합격 판정 토글 | 아톰 | 없음 | 3 | |
...
1. **티어 1**(25개, 이미 디자인 있음)부터 코드화를 시작한다.
2. 티어 1이 끝나면 **티어 2**(범용 필수 요소)로 넘어간다 — 화면을 만드는 데 실질적으로 막힘이 없어지는 지점.
3. **티어 3**(MES 핵심 특화)은 실제 업무 화면 완성도를 좌우하므로 전부 포함한다.
4. **티어 4**는 전부는 아니고 실무 임팩트가 큰 항목만 선별해서 포함한다 (아래 18-3 참고). **티어 5**는 실제 필요 여부를 재확인한 뒤 불필요하면 제외.
...
### 18-3. 티어 4 선별 기준 (39개 중 어디까지 포함할지)

티어 4가 39개로 많아서, 전부 넣기보다 아래 기준으로 1차 선별안을 제시. 확정은 아니고 조정 가능.
